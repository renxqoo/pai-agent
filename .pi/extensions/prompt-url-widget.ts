...
async function fetchAdvisoryMetadata(
  pi: ExtensionAPI,
  cwd: string,
  target: string,
...
export default function promptUrlWidgetExtension(pi: ExtensionAPI) {
  ...
  const applySessionName = (ctx: ExtensionContext, match: PromptMatch, metadata?: GhMetadata) => {
    ...
    const desiredName = trimmedTitle
      ? `${label}: ${trimmedTitle} (${displayTarget})`
      : desiredFallbackName;
    ...
    if (
      currentName === match.target ||
      currentName === fallbackName ||
      currentName === desiredFallbackName
    ) {
      pi.setSessionName(desiredName);
    }
  };
