...
function parseRef(
  ...
  if (issueMatch)
    return { type: "issue", owner: issueMatch[1], repo: issueMatch[2], issue: issueMatch[3] };

  if (GIST_ID_RE.test(ref)) return { type: "gist", id: ref };

  throw new Error(
    `expected a gist ID, gist URL, pi.dev share URL, issue URL, .html file, or .jsonl file: ${ref}`,
  );
}
...
async function findIssueGistId(owner: string, repo: string, issue: string): Promise<string> {
  ...
  if (!gistId)
    throw new Error(`no github-actions gist link found in comments on ${owner}/${repo}#${issue}`);
  return gistId;
}
    ...
    handler: async (args: string, ctx: ExtensionCommandContext) => {
      ...
      try {
        ...
        if (parsedRef.type === "gist") {
          ...
          if (!existsSync(parsedRef.path))
            throw new Error(`session file not found: ${parsedRef.path}`);
          const raw = readFileSync(parsedRef.path, "utf8");
          decoded = parsedRef.path.endsWith(".html")
            ? decodeExportedHtml(raw)
        ...
        ctx.ui.notify(
          `Imported session ${decoded.header.id} (cwd ${decoded.header.cwd} -> ${targetCwd})`,
          "info",
        );
