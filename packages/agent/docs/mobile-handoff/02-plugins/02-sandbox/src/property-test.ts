...
function randomJson(depth = 0): unknown {
  ...
  if (r < 0.8)
    return Array.from({ length: Math.floor(Math.random() * 5) }, () => randomJson(depth + 1));
...
c.load(
  `globalThis.churn = function () { for (var i = 0; i < 500; i++) __l.drop(function () { return i; }); };`,
);
c.load(`churn();`);
...
await c.endow("__q", {
  show: (v: string) => check("host cannot pollute guest prototype", v === "undefined", v),
});
...
});
{
  const t0 = Date.now();
  try {
    c.load(`globalThis.__x = __s.run(spin);`, 50);
  } catch {
    /* expected */
  }
  const took = Date.now() - t0;
  check(
    "re-entrant call uses its OWN budget, not the outer one",
    took > 1000,
...
await c.endow("__o", {
  show: (v: string) => check("async host result reaches guest", v === '"v"', v),
});
