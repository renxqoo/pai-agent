...
// 1. Are two CONTEXTS in one isolate isolated from each other?
{
  ...
  const a = iso.createContextSync(),
    b = iso.createContextSync();
  ...
  try {
    a.evalSync(`while(true){}`, { timeout: 100 });
  } catch {
    /* expected */
  }
  ...
  try {
    a.evalSync(`var x=[]; for(;;) x.push(new Array(50000).fill(1));`, { timeout: 15000 });
  } catch (e: any) {
    console.log("A bomb:", e.message.slice(0, 45));
  }
  try {
    console.log("B after A's bomb:", b.evalSync(`1+1`));
  } catch (e: any) {
    console.log("B after A's bomb: DEAD -", e.message.slice(0, 40));
  }
  try {
    iso.dispose();
  } catch {}
...
for (const mode of ["one isolate, N contexts", "N isolates"]) {
  ...
  console.log(
    `50 facets, ${mode.padEnd(24)}: ${(((rss() - before) / 50) * 1024).toFixed(0)} KB each, ${Date.now() - t} ms`,
  );
  if (shared) {
    try {
      shared.dispose();
