...
// ── isolate creation ──────────────────────────────────────────────────────
{
  ...
  console.log(
    `50 isolates:                  ${((process.memoryUsage().rss - before) / 50 / 1024).toFixed(0)} KB each, ${Date.now() - t} ms total`,
  );
...
// ── real Markdown at transcript scale ─────────────────────────────────────
{
  ...
  const t = Date.now();
  ctx.evalSync(readFileSync(new URL("./markdown-bundle.js", import.meta.url), "utf8"), {
    timeout: 60000,
  });
