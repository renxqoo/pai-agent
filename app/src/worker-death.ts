...
function settleClosedWorker(deps: DeathDeps, worker: WorkerHandle, reason: string): void {
  ...
  if (!wasRetire) {
    ...
    emitThreadParkedIfPresent(deps, worker);
  }
}
 ...
 * Skipped when the non-live cap just evicted the entry: a parked frame for a
 * vanished entry would contradict the next thread/list reconciliation. */
function emitThreadParkedIfPresent(deps: DeathDeps, worker: WorkerHandle): void {
  if (deps.table.entry(worker.threadId) === undefined) return;
  deps.emitFrame({
    type: "thread_parked",
