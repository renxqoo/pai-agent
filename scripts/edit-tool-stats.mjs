...
function classifyErrorKind(text, isError, matchedResult) {
  ...
  if (
    normalized.includes("found multiple occurrences") ||
    /^found \d+ occurrences/m.test(normalized)
  ) {
  ...
  const buckets = [
    {
      ...
      test: (record) => record.inflationRatio === null,
    },
    {
      key: "lt4",
      label: "<4x",
      test: (record) => record.inflationRatio !== null && record.inflationRatio < 4,
    },
    {
      key: "4to10",
      label: "4-10x",
      test: (record) =>
        record.inflationRatio !== null && record.inflationRatio >= 4 && record.inflationRatio < 10,
    },
    {
      key: "10to25",
      label: "10-25x",
      test: (record) =>
        record.inflationRatio !== null && record.inflationRatio >= 10 && record.inflationRatio < 25,
    },
    {
      key: "gte25",
      label: "25x+",
      test: (record) => record.inflationRatio !== null && record.inflationRatio >= 25,
    },
  ];
...
function buildSummary(records, meta, options) {
  const uniqueAssistantMessages = new Set(
  ...
  const multiEditLengthBuckets = [
    {
      key: "1",
      label: "edits.length === 1",
      test: (record) => record.mode === "multi" && record.editsCount === 1,
    },
    {
      key: "2",
      label: "edits.length === 2",
      test: (record) => record.mode === "multi" && record.editsCount === 2,
    },
    {
      key: "3plus",
      label: "edits.length >= 3",
      test: (record) => record.mode === "multi" && record.editsCount >= 3,
    },
  ].map((bucket) => {
    const bucketRecords = records.filter(bucket.test);
  ...
  const pathologicalThresholds = [10, 25, 100].map((threshold) => ({
    ...
    count: records.filter(
      (record) => record.inflationRatio !== null && record.inflationRatio >= threshold,
    ).length,
  }));
  const failureKinds = [
    ...groupCounts(
  ...
  return {
    ...
    scan: {
      ...
      since: meta.since
        ? { ms: meta.since.ms, iso: formatIso(meta.since.ms), source: meta.since.source }
        : null,
...
function printHumanReport(summary) {
  ...
  console.log(
    `Scanned ${formatInt(scan.sessionFilesIncluded)} session files in ${scan.sessionsDir}`,
  );
  if (scan.since) {
    console.log(
      `Session filter: files created at or after ${scan.since.iso} (${scan.since.source})`,
    );
    console.log(
  ...
  console.log(
    `  failed:     ${formatInt(counts.failed)}  ${formatPercent(counts.failed, counts.resolvedEditCalls)}`,
  );
  ...
  for (const entry of inflation.hugeReplacements) {
    console.log(
      `  >${formatBytes(entry.threshold).padEnd(6)} ${formatInt(entry.count).padStart(8)}`,
    );
  }
  ...
  for (const entry of inflation.pathologicalThresholds) {
    console.log(
      `  inflation >= ${String(entry.threshold).padEnd(3)}x ${formatInt(entry.count).padStart(8)}`,
    );
  }
  ...
  console.log(
    `  total same-file clusters:                      ${formatInt(sameFileClusters.clustersCount)}`,
  );
  console.log(
    `  calls inside those clusters:                  ${formatInt(sameFileClusters.callsInsideClusters)}`,
  );
  console.log(
...
function applyFilters(records, options) {
  return records.filter((record) => {
    if (
      options.modelFilter &&
      !record.providerModel.toLowerCase().includes(options.modelFilter.toLowerCase())
    ) {
      return false;
    }
