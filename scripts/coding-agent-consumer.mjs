...
const developmentPackages = new Set(
  ["pi-client", "pi-protocol", "pi-server"].map((name) => `@earendil-works/${name}`),
);
...
export function installCodingAgentConsumer(directory, tarballs, packageManager = "npm") {
  ...
  const installArgs =
    packageManager === "bun" ? ["--production"] : ["--omit=dev", "--no-audit", "--no-fund"];
...
function checkInstalledPackages(nodeModules, seen = new Set()) {
  ...
  for (const directory of directories) {
    ...
    if (developmentPackages.has(manifest.name))
      throw new Error(`${manifest.name} must not be installed: ${path}`);
...
`,
    ...
    for (const cli of new Set([manifest.bin.pi, "dist/cli.js"])) {
      const output = run(runtime, [join(packageDir, cli), "--version"], {
        cwd: directory,
        env,
        timeout: 30_000,
      });
      if (output.trim() !== manifest.version)
        throw new Error(`Unexpected version from ${cli}: ${output}`);
    }
  } finally {
