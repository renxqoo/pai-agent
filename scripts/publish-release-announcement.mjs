...
async function verifyPackagesAreAvailable(packages) {
  ...
  do {
    ...
    if (failures.length === 0) {
      console.log(
        `All ${packages.length} Pi packages are available from npm (attempt ${attempt}).`,
      );
      return results.map((result) => result.value);
    }

    console.log(
      `Waiting for ${failures.length} Pi package${failures.length === 1 ? "" : "s"} on npm (attempt ${attempt}):`,
    );
  ...
  throw new Error(
    `Timed out waiting for Pi packages to become available from npm:\n${failures.map((failure) => `  ${failure}`).join("\n")}`,
  );
}
...
function runAws(args, { allowNotFound = false, allowPreconditionFailure = false } = {}) {
  ...
  if (
    allowPreconditionFailure &&
    /(?:412|PreconditionFailed|ConditionalRequestConflict)/i.test(message)
  ) {
    return undefined;
  }
...
export function compareReleaseVersions(left, right) {
  ...
  if (!leftMatch || !rightMatch)
    throw new Error("Release versions must be stable semver versions.");
...
export async function advanceLatestRelease(version, readLatest, writeLatest) {
  ...
  throw new Error(
    `Could not advance the Pi release marker to ${version} after ${MAX_POINTER_UPDATE_ATTEMPTS} attempts.`,
  );
}
