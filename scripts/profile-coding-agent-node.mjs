...
async function runTuiBenchmarkRun({ runtime, runIndex, measuredIndex, options, profileDir }) {
  ...
  const tempRoot = options.isolatedAgentDir
    ? mkdtempSync(join(tmpdir(), "pi-startup-benchmark-"))
    : undefined;
...
async function main() {
  ...
  const entryPath =
    runtime === "bun" ? srcCliPath : options.bundle ? bundledDistCliPath : distCliPath;
  if (
    runtime === "node" &&
    !options.bundle &&
    !options.build &&
    ...
    throw new Error(
      "dist/cli.js is a bundled facade; rerun without --skip-build for an unbundled profile",
    );
  }
  ...
  const maxElapsedRun = measuredRuns.reduce((slowest, run) =>
    run.elapsedMs > slowest.elapsedMs ? run : slowest,
  );
