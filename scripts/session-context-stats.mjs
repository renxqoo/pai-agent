...
function parseSessionFileTimestamp(sessionFile) {
  ...
  const ms = Date.parse(
    rawTimestamp.replace(/T(\d{2})-(\d{2})-(\d{2})-(\d{3})Z$/, "T$1:$2:$3.$4Z"),
  );
...
async function scanSessions(sessionsDir, sinceMs, contextWindows, cwdFilter) {
  ...
  for await (const sessionFile of walkJsonlFiles(sessionsDir)) {
    ...
    for await (const line of rl) {
      ...
      if (message.role === "assistant" && Array.isArray(message.content)) {
        for (const block of message.content) {
          ...
          const command =
            typeof block.arguments?.command === "string" ? block.arguments.command : "";
    ...
    if (session.preFirstCompactionTokens !== null && session.contextWindow !== null) {
      session.preFirstCompactionUsagePercent =
        (session.preFirstCompactionTokens / session.contextWindow) * 100;
    }
...
function summarizeSessionGroup(key, group) {
  ...
  const withPreCompactionUsage = group.filter(
    (session) => session.preFirstCompactionUsagePercent !== null,
  );
  ...
  return {
    ...
    avgDurationMinutes:
      group.reduce(
        (sum, session) => sum + Math.max(0, session.endMs - session.startMs) / 60000,
        0,
      ) / group.length,
    avgMaxPromptTokens:
      withTokens.length === 0
        ? null
        : withTokens.reduce((sum, session) => sum + (session.maxPromptTokens ?? 0), 0) /
          withTokens.length,
    medianMaxPromptTokens: median(withTokens.map((session) => session.maxPromptTokens)),
    avgMaxContextUsagePercent:
      withUsage.length === 0
        ? null
        : withUsage.reduce((sum, session) => sum + (session.maxContextUsagePercent ?? 0), 0) /
          withUsage.length,
    medianMaxContextUsagePercent: median(
      withUsage.map((session) => session.maxContextUsagePercent),
    ),
...
function buildSummary(sessions, meta, options) {
  ...
  const filtered = sessions.filter((session) => {
    ...
    if (options.modelFilter && !providerModel.includes(options.modelFilter.toLowerCase()))
      return false;
    if (
      lowerPrefixes.length > 0 &&
      !lowerPrefixes.some((prefix) => providerModel.startsWith(prefix))
    )
      return false;
    if (bashContains.length > 0) {
      const commands = session.bashCommands.map((command) => command.toLowerCase());
      if (!bashContains.some((text) => commands.some((command) => command.includes(text))))
        return false;
    }
    return true;
  });
...
function buildTextReport(summary) {
  const lines = [];
  lines.push(
    `Scanned ${formatInt(summary.scan.sessionFilesIncluded)} session files in ${summary.scan.sessionsDir}`,
  );
...
async function main() {
  ...
  const { sessions, meta } = await scanSessions(
  ...
  const summary = buildSummary(
    sessions,
    { ...meta, contextWindowSources: contextWindows.sources },
    options,
  );
