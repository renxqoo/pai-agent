...
function formatJsonForDiff(value, indent = "") {
  ...
  return `{\n${entries
    .map(
      ([key, entry]) =>
        `${childIndent}${JSON.stringify(key)}: ${formatJsonForDiff(entry, childIndent)},`,
    )
    .join("\n")}\n${indent}}`;
}
...
try {
  ...
  if (thinkingOnly) {
    ...
    generateThinkingCatalog(
      baselineWorktree,
      join(baselineOutput, "models.json"),
      baselineThinkingOutput,
    );
  ...
  const providers =
    requestedProviders.length > 0
      ? requestedProviders
      : [...new Set([...beforeProviders, ...afterProviders])].sort();
  ...
  for (const provider of providers) {
    ...
    const modelIds = [
      ...new Set([...Object.keys(beforeModels ?? {}), ...Object.keys(afterModels ?? {})]),
    ].sort();
    for (const modelId of modelIds) {
      const beforeModel = beforeModels?.[modelId];
      const afterModel = afterModels?.[modelId];
      if (
        JSON.stringify(canonicalizeJson(beforeModel)) ===
        JSON.stringify(canonicalizeJson(afterModel))
      )
        continue;
      ...
      if (result.status === 1) {
        ...
        throw new Error(
          `Could not compare ${provider}/${modelId}: ${result.stderr || result.stdout}`,
        );
      }
    }
  }
  ...
  if (differences === 0) {
    console.log(
      `No model catalog changes${requestedProviders.length === 1 ? ` for ${requestedProviders[0]}` : ""}.`,
    );
  } else {
