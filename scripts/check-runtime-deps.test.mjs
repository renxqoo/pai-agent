...
test("rejects undeclared imports even when the workspace package exists", async (t) => {
  const result = await check(
    t,
    {},
    'export { createUnixServer } from "@earendil-works/pi-server/unix";',
    {
  ...
  assert.match(
    result.stderr,
    /src[\\/]index\.ts:1: @earendil-works\/pi-server\/unix is not declared/,
  );
});
...
test("allows imported JSON assets outside the TypeScript include pattern", async (t) => {
  const result = await check(t, {}, 'import data from "./data.json";', {
    "packages/example/tsconfig.build.json": JSON.stringify({
      include: ["src/**/*.ts"],
...
for (const statement of [
  'export * from "./experimental/server";',
  'import type { Options } from "./experimental/server";',
]) {
