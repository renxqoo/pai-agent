...
function parseArgs(args) {
  ...
  if (!options.dryRun && !options.endpoint)
    throw new Error("--endpoint is required when publishing");
  return options;
}
...
function validateBundle(inputDir) {
  ...
  for (const providerId of REQUIRED_PROVIDERS) {
    if (!Object.hasOwn(models, providerId))
      throw new Error(`Required provider is missing: ${providerId}`);
  }
  ...
  const shardFiles = readdirSync(providersDir)
  ...
  if (modelCount < MINIMUM_MODEL_COUNT) {
    throw new Error(
      `Refusing to publish only ${modelCount} models; expected at least ${MINIMUM_MODEL_COUNT}`,
    );
  }
...
function validateIndex(index) {
  ...
  if (!Array.isArray(index.catalogs))
    throw new Error(`Existing ${CATALOG_INDEX_KEY} has no catalogs array`);
...
async function main() {
  ...
  try {
    ...
    if (
      currentIndex?.defaultRevision === bundle.revision &&
      currentEntry?.revision === bundle.revision
    ) {
