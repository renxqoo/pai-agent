...
const DEFAULT_ACTIVE_READ_TOOL_PATH = path.join(
  process.cwd(),
...
async function resolveAutoSinceMs(options) {
  ...
  try {
    ...
    return Number.isFinite(ms) && ms > 0
      ? { ms, source: `birth time of ${options.autoSincePath}` }
      : null;
  } catch {
    return null;
  }
...
function classifyRead(args) {
  ...
  const hasOffset =
    Object.hasOwn(normalizedArgs, "offset") &&
    normalizedArgs.offset !== undefined &&
    normalizedArgs.offset !== null;
  const hasLimit =
    Object.hasOwn(normalizedArgs, "limit") &&
    normalizedArgs.limit !== undefined &&
    normalizedArgs.limit !== null;
  return {
...
function summarizeTimeBuckets(records, bucket) {
  return summarizeGroups(records, (record) => getTimeBucket(record.timestampMs, bucket)).sort(
    (a, b) => a.key.localeCompare(b.key),
  );
}

function summarizeNormalizedTimeBuckets(records, bucket) {
  return summarizeNormalizedTimeBucketsByKey(records, (record) =>
    getTimeBucket(record.timestampMs, bucket),
  );
}
  ...
  return [...groups.entries()]
    .map(([key, group]) => {
      ...
      const assistantMessages = new Set(
        group.map((record) => `${record.sessionFile}::${record.assistantEntryId}`),
      ).size;
      return {
        key,
        reads: group.length,
...
function buildSummary(records, meta, options) {
  ...
  const timeOfDayStats = summarizeGroups(records, (record) =>
...
function printHumanReport(summary) {
  ...
  for (const group of timeStatsByProvider) {
    console.log(`\n${group.providerModel}`);
    console.log(
      `  total reads=${formatInt(group.reads)} assistantMessages=${formatInt(group.assistantMessages)}`,
    );
    console.log(
...
async function main() {
  ...
  if (options.json) {
    console.log(
      JSON.stringify(
        options.includeRecords ? { summary, records: filteredRecords } : { summary },
        null,
        2,
      ),
    );
