...
function checkSource(source, manifest) {
  ...
  function checkSpecifier(node) {
    ...
    failures.push(
      `${file}:${line + 1}: ${specifier} is not declared in ${manifest.name}'s runtime dependencies`,
    );
  }
...
for (const { directory } of getPublicWorkspacePackages()) {
  ...
  for (const source of program.getSourceFiles()) {
    ...
    if (!roots.has(resolve(source.fileName))) {
      failures.push(
        `${source.fileName} is excluded from ${manifest.name}'s build but imported by it`,
      );
    }
