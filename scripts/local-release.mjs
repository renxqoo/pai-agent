  ...
  if (process.platform === "win32") {
    if (existsSync(join(binDirectory, "pi.cmd"))) {
      ...
      writeFileSync(
        ...
        '& "$PSScriptRoot/node_modules/.bin/pi.ps1" @args\n',
      );
      return;
    }
    writeFileSync(
    ...
    writeFileSync(
      join(installDirectory, "pi.ps1"),
      '& "$PSScriptRoot/node_modules/.bin/pi.exe" @args\n',
    );
    return;
  }
...
for (const pkg of packages) {
  ...
  run("npm", ["run", pkg.directory === "packages/ai" ? "build:offline" : "build"], {
    cwd: pkg.directory,
  });
}
...
if (!options.skipInstall) {
  ...
  if (!options.skipBunInstall) {
    if (!commandExists("bun")) {
      throw new Error(
        "Bun is required for the isolated Bun install. Use --skip-bun-install to skip it.",
      );
    }
...
if (!options.skipInstall) {
  ...
  console.log(
    `  ${join(binaryDirectory, String(binaryPlatform).startsWith("windows-") ? "pi.exe" : "pi")} --help`,
  );
  ...
  console.log(
    `  ${join(nodeInstallDirectory, process.platform === "win32" ? "pi.cmd" : "pi")} --help`,
  );
  ...
  if (!options.skipBunInstall) {
    ...
    console.log(
      `  ${join(bunInstallDirectory, process.platform === "win32" ? "pi.cmd" : "pi")} --help`,
    );
  }
}
